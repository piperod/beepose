...
def extract_patch(im, params,translation=False,rotation=False,scale=False, tform=None, mode='float64'):
    ...
    #print(output_shape)
    if (mode is 'float64'):
        im_out = warp(im, tform, mode='constant',cval=0.5, output_shape=output_shape)
        #im_out = warp(im, tform, mode='constant', cval=0.5)
    elif (mode is 'int64'):
...
def get_augemented_image_and_label(im, lab, output_shape=(368, 368), translation=True, scale=True, rotation=True, mins=0.25,maxs=1.2, ilumination=0.0):
...
def get_augmented_image(meta):
    ...
    params = random_tform_params((meta.height,meta.width),thoraxes, output_shape=meta.output_shape, mins=meta.mins,maxs=meta.maxs)
