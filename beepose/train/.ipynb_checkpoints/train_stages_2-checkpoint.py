...
import numpy as np
from data.pose_dataset import get_dataflow,get_dataflow_batch
from data.pose_augmenter import set_network_input_wh, set_network_scale
...
def gen(df,batch_size,stages):
    ...
    while True:
        ...
        data_img, heat_maps,vector_maps  = tuple(next(df.get_data()))
        batch_x=data_img
        batch_x1=np.ones(heat_maps.shape)
        batch_x2=np.ones(vector_maps.shape)
        array=[heat_maps, vector_maps,
               heat_maps, vector_maps,
               heat_maps, vector_maps,
               heat_maps, vector_maps,
               heat_maps, vector_maps,
               heat_maps, vector_maps]
        
        yield [batch_x, batch_x1,  batch_x2], array[:stages*2]
        
def prepare_generators(ann_path,imgs_path,network_scale=8,image_resize=4):
    # prepare generators
    
    set_network_input_wh(2560//image_resize, 1440//image_resize)
    ...
    batch_df = get_dataflow_batch(ann_path,True,batch_size,imgs_path)
...
if __name__ == '__main__':
    ...
    train_gen,train_samples = prepare_generators(ann_path=str(args.ann),imgs_path=str(args.imgs))
