...
class DataGeneratorClient(object):
    ...
    def __init__(self, host, port, hwm=20, batch_size=10,np1=38,np2=19,stages=6):
        """
        :param host:
        :param port:
        :param hwm:, optional
    ...
    def gen(self):
        ...
        while True:
            ...
            if sample_idx == self.batch_size:
                ...
                np1=self.np1
                np2=self.np2
                array=[batch_y1[:,:,:,:np1], batch_y2[:,:,:,:np2],
                        batch_y1[:,:,:,:np1], batch_y2[:,:,:,:np2],
                        batch_y1[:,:,:,:np1], batch_y2[:,:,:,:np2],
                        batch_y1[:,:,:,:np1], batch_y2[:,:,:,:np2],
                        batch_y1[:,:,:,:np1], batch_y2[:,:,:,:np2],
                        batch_y1[:,:,:,:np1], batch_y2[:,:,:,:np2]]
                #import pdb; pdb.set_trace()
                yield [batch_x, batch_x1[:,:,:,:np1],  batch_x2[:,:,:,:np2]], array[:self.stages*2]
